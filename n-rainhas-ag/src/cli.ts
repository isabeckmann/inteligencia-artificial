...
import { AlgoritmoGenetico, configPadrao, type Cruzamento, type Selecao } from "./ga.js";

const selecoes: Selecao[] = ["roleta", "torneio", "ranking"];
const cruzamentos: Cruzamento[] = ["um-ponto", "dois-pontos", "uniforme"];
const EXECUCOES = 20;
...
for (const selecao of selecoes)
  for (const cruzamento of cruzamentos) {
    ...
    for (let e = 0; e < EXECUCOES; e++) {
      ...
      for (;;) {
        const g = ag.passo();
        if (g.fim) { if (g.resolvido) { ok++; soma += g.numero; } break; }
      }
    }
